...
const seat_history_1 = require("./../src/model/seat-history");
...
describe('multi-player-table-fixture', () => {
    ...
    let setup3PlayerGame = function () {
        ...
        test_helpers_1.TestHelpers.addPlayerHandle(table, 3, subscriber2);
    ...
    let getPlayerHandle = (subscriber) => {
    ...
    let setup4PlayerGame = function () {
        ...
        test_helpers_1.TestHelpers.addPlayerHandle(table, 7, subscriber4);
    ...
    let assertBroadcast = function (assertFunc, sockets = null) {
    ...
    it('3 player dealHoleCards', function () {
        table.subscribers.push(subscriber2);
        table.subscribers.push(subscriber3);
        table.subscribers.push(subscriber4);
        table.handleJoinTableRequest(test_helpers_1.TestHelpers.getJoinTableRequest(1, subscriber));
        table.handleJoinTableRequest(test_helpers_1.TestHelpers.getJoinTableRequest(3, subscriber2));
        table.handleJoinTableRequest(test_helpers_1.TestHelpers.getJoinTableRequest(5, subscriber3));
        socket1.clearMessages();
    ...
    it('4 players and 1st to act is seat 4 ', function () {
        ...
        test_helpers_1.TestHelpers.addPlayerHandle(table, 3, subscriber2);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 5, subscriber3);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 8, subscriber4);
        table.setCurrentPlayers(table.getPlayers().slice());
        table.dealerSeat = 1;
        ...
        table.gameState.pots.push(new TexasHoldemGameState_1.GamePot());
        table.gameState.pots[0].amount = 3;
        table.gameState.pots[0].players.push(table.currentPlayers.find(p => p.seat === 3), table.currentPlayers.find(p => p.seat === 5));
    ...
    let setup4PlayerShowdown = function () {
        ...
        table.subscribers.push(subscriber4);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 1, subscriber);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 4, subscriber2);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 6, subscriber3);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 7, subscriber4);
        table.setCurrentPlayers(table.getPlayers().slice());
        ...
        table.gameState.pots.push(new TexasHoldemGameState_1.GamePot());
        table.gameState.pots[0].players.push(table.currentPlayers[0], table.currentPlayers[1], table.currentPlayers[3], table.currentPlayers[2]);
    ...
    it('4 players showdown', async () => {
        ...
        table.dataRepository.receivedWith('saveTableStates', substitute.arg.matchUsing((arg) => {
            if (arg == undefined)
                return false;
    ...
    it('4 players showdown where winning hand folds', async () => {
        setup4PlayerShowdown();
    ...
    it('4 players showdown with collapsing pot', async () => {
        setup4PlayerShowdown();
        let dataRepository = table.dataRepository;
        ...
        dataRepository.receivedWith('saveGame', substitute.arg.matchUsing(function (arg) {
            return arg != undefined && arg.potResults[0].allocations[0].player.guid === 'guid4';
        }));
        dataRepository.receivedWith('updateTableBalances', 'id1', 'usd', substitute.arg.matchUsing((arg) => {
            return arg != undefined && arg.length === 4 && arg[3].balance === 1150;
        }));
    });
    it('4 players showdown with other players folding', async () => {
        setup4PlayerShowdown();
        table.currentPlayers.find(p => p.seat === 1).hasFolded = true;
        table.currentPlayers.find(p => p.seat === 1).cumulativeBet = 0;
        table.currentPlayers.find(p => p.seat === 4).hasFolded = true;
        ...
        table.dataRepository.receivedWith('saveGame', substitute.arg.matchUsing(function (arg) {
            return arg != undefined && arg.tableId === table.tableConfig._id && arg.potResults[0].allocations[0].player.guid === 'guid4';
        }));
    });
    it('player folds after re-raise - original raiser call amount should not equal player that folded', function () {
        setup3PlayerGame();
    ...
    it('player goes all in less than the call amount', function () {
        setup3PlayerGame();
        table.currentPlayers[0].bet = 10;
        table.currentPlayers[1].bet = 20;
    ...
    it('2 all in players are skipped when player folds', function () {
        setup4PlayerGame();
        table.currentPlayers[1].stack = 0;
    ...
    it('player raising after all-in player sets last to act index as the player before the all in player', function () {
        setup3PlayerGame();
    ...
    let getPlayerTableHandle = (user, seat) => {
        return new PlayerTableHandle_1.PlayerTableHandle(user.guid, user.screenName, user.guid, seat);
    };
    ...
    it('play is started without sitting out player', function () {
    ...
    it('deal hole cards does not proceed when minimum number of players is not met', function () {
        table.minNumPlayers = 2;
        table.subscribers.push(subscriber2);
        table.subscribers.push(subscriber3);
        table.getPlayers().push(getPlayerTableHandle(subscriber.user, 1));
    ...
    it('player who is not playing broadcasts their sitting out status immediately', function () {
        setup3PlayerGame();
        table.subscribers.push(subscriber4);
    ...
    it('player who is playing does not broadcast their sitting out status', function () {
        setup3PlayerGame();
        var setTableOptionRequest = new ClientMessage_1.SetTableOptionRequest(undefined);
        setTableOptionRequest.sitOutNextHand = true;
        table.handleSetTableOptionRequest(setTableOptionRequest, subscriber2.user.guid);
    ...
    it('autofold after bet', function () {
        setup4PlayerGame();
        ...
        table.handleBet(2, subscriber.user.guid);
        timerProvider.receivedWith('startTimer', substitute.arg.matchUsing((func) => { return func != undefined && func.name === 'startAutoFoldTimer'; }), substitute.arg.matchUsing(function (a) { return a == 500; }), table);
        assert.equal(table.currentPlayers[table.playerNextToActIndex].seat, 3);
        assert.equal(getPlayerHandle(subscriber2).autoFold, false);
    });
    it('autofold after fold', function () {
        setup4PlayerGame();
        let request = new ClientMessage_1.SetTableOptionRequest(undefined);
        request.autoFold = true;
        table.handleSetTableOptionRequest(request, subscriber2.user.guid);
    ...
    it('player who is autofolding has autofold unchecked after action folds to them', () => {
        ...
        let autoOptionResult = CommonHelpers_1.CommonHelpers.allowAutoFold(table.currentPlayers[3].guid, table.currentPlayers);
    ...
    it('player pays big blind when moving seats', () => {
        ...
        for (let player of table.currentPlayers)
            player.bet = 0;
        table.pastPlayers = [[new seat_history_1.SeatHistory(subscriber.user.guid, 1),
                new seat_history_1.SeatHistory(subscriber3.user.guid, 5),
                new seat_history_1.SeatHistory(subscriber4.user.guid, 7),
                new seat_history_1.SeatHistory(subscriber2.user.guid, 8),
            ]];
        table.dealHoleCards();
        assert.equal(table.dealerSeat, 5);
        assert.equal(table.currentPlayers.find(p => p.seat === 1).bet, 2);
        assert.equal(table.currentPlayers.find(p => p.seat === 3).bet, 2);
        assert.equal(table.currentPlayers.find(p => p.seat === 5).bet, 0);
        assert.equal(table.currentPlayers.find(p => p.seat === 7).bet, 1);
        assert.equal(table.gameState.pots[0].amount, 5);
    });
    it('player who moves seat does become dealer', () => {
        ...
        table.dealerSeat = 8;
        table.pastPlayers = [[
                new seat_history_1.SeatHistory(subscriber2.user.guid, 3),
                new seat_history_1.SeatHistory(subscriber3.user.guid, 5),
                new seat_history_1.SeatHistory(subscriber4.user.guid, 7),
                new seat_history_1.SeatHistory(subscriber.user.guid, 8)
            ]];
        table.dealHoleCards();
    ...
    it('tournament sitting out players pay blinds', () => {
        ...
        test_helpers_1.TestHelpers.addPlayerHandle(table, 5, subscriber3);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 7, subscriber4);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 8, subscriber5);
        table.getPlayers().find(p => p.seat === 3).sitOutNextHand = true;
        table.getPlayers().find(p => p.seat === 5).isDisconnected = true;
        table.getPlayers().find(p => p.seat === 7).isDisconnected = true;
        table.dealerSeat = 1;
    ...
    it('dealCommunityCards with dealer who is sitting out', () => {
        ...
        let sockets = table.subscribers.map(s => s.socket);
        assertBroadcast(data => {
            for (let seat of data.tableSeatEvents.seats) {
                assert.equal(seat.playercards, undefined);
            }
    ...
    it('must cycle thru multiple sitting out players to get next to act', () => {
        ...
        test_helpers_1.TestHelpers.addPlayerHandle(table, 1, subscriber);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 3, subscriber2);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 4, subscriber5);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 5, subscriber3);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 7, subscriber4);
        table.getPlayers().find(p => p.seat === 1).sitOutNextHand = true;
    ...
    it('player is not removed from table postShowdown when playing a tournament when their stack is less than the blinds', async () => {
        ...
        table.broadcastService = substitute.for(new IBroadcastService_1.IBroadcastService());
        let dataRepository = substitute.for(new IDataRepository_1.IDataRepository());
        dataRepository.returnsFor('getUser', Promise.resolve(new User_1.User()));
        table.dataRepository = dataRepository;
        let event;
        table.onPostShowdown = (e) => {
            event = e;
    ...
    it('2 players deal hole cards when other players sitting out', () => {
        ...
        table.tournamentId = 'tournamentId';
        test_helpers_1.TestHelpers.addPlayerHandle(table, 1, subscriber);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 2, subscriber2);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 3, subscriber3);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 4, subscriber4);
        table.getPlayerAtSeat(3).isDisconnected = true;
        table.getPlayerAtSeat(4).isDisconnected = true;
        table.dealHoleCards();
        assert.equal(table.playerNextToActIndex, 0);
    });
    let setupdealCommunityCardsWhenDealerIsRemovedDueToIdleness = (dealerSeat) => {
        table.minNumPlayers = 2;
    ...
    it('setupdealCommunityCardsWhenDealerIsRemovedDueToIdleness 1', () => {
        test_helpers_1.TestHelpers.addPlayerHandle(table, 2, subscriber);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 3, subscriber2);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 4, subscriber4);
    ...
    it('blind of sitting out player is added to pot', async () => {
        ...
        let dbState = dataRepository.argsForCall('saveTableStates', 0)[0][0];
    ...
    it('sitting out player pays blind that is bigger than other players', async () => {
        ...
        await table.handleShowdown();
        let dbGame = dataRepository.argsForCall('saveGame', 0)[0];
    ...
    it('player wins as other players fold', async () => {
        ...
        test_helpers_1.TestHelpers.addPlayerHandle(table, 3, subscriber3);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 4, subscriber4);
        test_helpers_1.TestHelpers.addPlayerHandle(table, 7, subscriber5);
        let seat1 = table.getPlayerAtSeat(1);
...
//# sourceMappingURL=multi-player-table-fixture.js.map