...
const poker_evaluator_1 = require("../src/poker-evaluator");
const TexasHoldemGameState_1 = require("../src/model/TexasHoldemGameState");
var _ = require('lodash');
const util = require('util');
describe('PokerEvaluator', function () {
    let getPlayer = function (holecards, cumulativeBet, hasFolded) {
        let player = {
            holecards: holecards,
    ...
    it('rankHand', function () {
        let result = poker_evaluator_1.PokerEvaluator.rankHand(['2S', '3S', '4S', '5S', '6S', '7S', '8S']);
        assert.equal(result.bestHand, '4S5S6S7S8S');
        assert.equal(result.handRankEnglish, 'Straight Flush');
        result = poker_evaluator_1.PokerEvaluator.rankHand(['2S', '3S', '4S', 'AH', '6S', '10D', '8C']);
        assert.equal(result.bestHand, '4SAH6S10D8C');
        assert.equal(result.handRankEnglish, 'High Card');
        result = poker_evaluator_1.PokerEvaluator.rankHand(['2S', '3S', '4S', 'AH', '6S', '10D', '8S']);
        assert.equal(result.bestHand, '2S3S4S6S8S');
        assert.equal(result.handRankEnglish, 'Flush');
    });
    it('play board', function () {
        let pot = new TexasHoldemGameState_1.GamePot();
        let state = new TexasHoldemGameState_1.TexasHoldemGameState();
    ...
    it('allocate pot for 2 players where 1 player has folded', function () {
        ...
        pot.players.push(player1);
        let player2 = getPlayer(["AH", "KH"], 0, true);
        pot.players.push(player2);
        state.pots.push(pot);
        let result = state.allocateWinners();
        assert.equal(result.potResults.length, 1);
    ...
    it('allocate 3 pots', function () {
        ...
        assert.equal(_.sumBy(state.pots, function (p) { return p.amount; }), 210);
    ...
    it('3 pots', function () {
        ...
        assert.equal(_.sumBy(state.pots, function (p) { return p.amount; }), 340);
    });
    ...
    it('complex pot', function () {
        ...
        assert.equal(_.sumBy(state.pots, function (p) { return p.amount; }), 105);
    });
    it('folded pot is moved to main pot', function () {
        let state = new TexasHoldemGameState_1.TexasHoldemGameState();
    ...
    it('rankHand-non-suited-flush', function () {
        let result1 = poker_evaluator_1.PokerEvaluator.rankHand(['5H', '6D', '2D', 'KH', '4S', '3S', '5C']);
        let result2 = poker_evaluator_1.PokerEvaluator.rankHand(['5H', '6D', '2D', 'KH', '4S', 'AC', '3D']);
    ...
    it('rankHand-suited-flush', function () {
        let result1 = poker_evaluator_1.PokerEvaluator.rankHand(['5C', '6C', '3C', '2C', '4C', 'AH', 'KH']);
        let result2 = poker_evaluator_1.PokerEvaluator.rankHand(['5C', '6C', '3C', '2C', '4C', 'AC', 'KH']);
        assert.equal(result1.score, 414770);
...
//# sourceMappingURL=poker-evaluator-fixture.js.map