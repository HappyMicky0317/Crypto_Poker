...
class ProtobufConfig {
    ...
    defined(data) {
        let protoMessageDefinition = this.root.lookupType(`shared.DataContainer`);
    ...
    serialize(data, type) {
        let protoMessageDefinition = this.root.lookupType(`shared.${type}`);
    ...
    deserialize(buffer, type) {
        let byteArray = buffer instanceof ArrayBuffer ? new Uint8Array(buffer) : new Uint8Array(buffer.buffer, buffer.byteOffset, buffer.length);
        let protoMessageDefinition = this.root.lookupType(`shared.${type}`);
...
//# sourceMappingURL=protobuf-config.js.map